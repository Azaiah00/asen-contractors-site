      ...
      >
        ...
        <div className="md:hidden flex items-center gap-3">
          <button
            ...
            className="px-3 py-2 bg-gold-600 text-black rounded-full text-[10px] font-semibold uppercase tracking-wider hover:bg-gold-500 transition-colors"
          >
            Book Now
          </button>
          {!isOpen && (
        ...
        <AnimatePresence>
          {isOpen && (
            <>
              ...
              >
                {/* Mobile Menu Content - Solid Black Background */}
                <div className="h-screen bg-black flex flex-col items-center px-5 pt-6 pb-10">
                  {/* Close Button */}
                  <button
                    onClick={() => setIsOpen(false)}
                  ...
                  <div className="mt-2">
                    <Link href="/" onClick={() => setIsOpen(false)}>
                      <div className="text-2xl font-serif text-white tracking-tighter">
                        ASEN<span className="text-gold-500">.</span>
                      </div>
                    </Link>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-4 mt-10">
                    {navLinks.map((link) => (
                      <Link 
                        key={link.name} 
                        href={link.href}
                        className="text-xl font-serif text-white hover:text-gold-500 transition-colors"
                        onClick={() => setIsOpen(false)}
                      >
                        {link.name}
                      </Link>
                  ...
                  <button
                    ...
                    className="bg-gold-600 text-black hover:bg-gold-500 px-8 py-3 rounded-full font-sans font-semibold text-xs uppercase tracking-widest transition-all duration-300 text-center mt-auto mb-4"
                  >
                    Book Now
                  </button>
                </div>
